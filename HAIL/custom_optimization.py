...
def compute_loss(loss_A_hard, loss_A_soft, loss_B_hard, loss_B_soft, alpha,global_step,num_train_steps):
    if alpha < 0:
        y = tf.cast(global_step//(int(num_train_steps*0.02)+1), dtype=tf.float32)
        alpha = tf.pow(0.98, y)
    loss = alpha * loss_A_hard + (1 - alpha) * loss_A_soft + alpha * loss_B_hard + (1 - alpha) * loss_B_soft
    return loss


def create_optimizer(loss_A_hard, loss_A_soft, loss_B_hard, loss_B_soft, alpha, init_lr, num_train_steps, num_warmup_steps,
                     use_tpu):
    ...
    loss = compute_loss(loss_A_hard, loss_A_soft, loss_B_hard, loss_B_soft, alpha,global_step,num_train_steps)
